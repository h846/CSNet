<template>
  <v-container class="py-8 px-6" fluid>
          ...
          <v-list rounded>
            <v-list-item-group color="primary">
              ...
              <v-list-item @click.stop="addDialog = true">
                <v-list-item-icon>
                  <v-icon class="deep-orange--text">mdi-plus-circle-outline</v-icon>
                </v-list-item-icon>
                <v-list-item-content>
              ...
              <v-list-item @click.stop="dialog = true">
                <v-list-item-icon>
                  <v-icon class="light-blue--text">mdi-minus-circle-outline</v-icon>
                </v-list-item-icon>
                <v-list-item-content>
    ...
    <CatAddDialog :catlist="category" :addDialog="addDialog" @closeAddDialog="addDialog = false" />
    <LinkListDialog :catlist="category" :dialog="dialog" @closeDialog="dialog = false" />
  </v-container>
</template>
<script>
  ...
  methods: {
    sortEnd() {
      ...
      for (const index in this.category) {
        let [position, id] = [this.category[index].position, this.category[index].ID];
